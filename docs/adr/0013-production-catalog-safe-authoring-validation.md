...
- The validated ProjectBundle in the selected Project folder is the user asset source. The latest JSON is atomically replaced after a two-second edit burst, with at most 50 prior versions; app config stores the path preference, while localStorage is only a recovery shadow and current-workspace cache.
