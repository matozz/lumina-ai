...
- Status: Accepted
- Date: 2026-08-02
- Amended: 2026-08-23 for ordered same-tick automation boundaries
...
- CueClip 使用半开区间；结束 tick 不 active。AutomationLane keyframe tick 非递减；同 tick 的稳定数组顺序定义瞬时边界，第一个点是左极限目标，最后一个点从该 tick 起 active。continuous 参数按声明插值，discrete 参数使用 hold。
...
- 瞬时切换不需要伪造 `boundary + 1 tick`；validation、compiler、直接 Seek 和积分求值必须保留并消费同 tick 顺序。
