    ...
    pub fn compile_project(
        ...
        for cue in &cues {
            ...
            for layer in &cue.layers {
                ...
                instances.push(effect_instance(&instance_id, layer, None));
                customizations.insert(instance_id.clone(), customization(layer, None));
                compiled_layers.push(CompiledCueLayer {
                    id: layer.id.clone(),
                    instance: instance_id.into(),
                    target: target_group_id(&stage, &layer.target_set_ref.target_set_id).into(),
        ...
        for (track_index, track) in arrangement.tracks.iter().enumerate() {
            ...
            for clip in &track.clips {
                ...
                for layer in &cue.layers {
                    ...
                    instances.push(effect_instance(&instance_id, layer, layer_override));
                    customizations
                        .insert(instance_id.clone(), customization(layer, layer_override));
...
fn effect_instance(
    ...
    EffectInstanceDSL {
        ...
        target_group_id: format!(
            "__target_set__:{}:{}:{}:{}",
            layer.target_set_ref.stage_id.len(),
            layer.target_set_ref.stage_id,
            layer.target_set_ref.stage_revision,
